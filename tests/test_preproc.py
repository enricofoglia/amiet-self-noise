...
def test_wps():
    ...
    pressure = [
            np.sin(2 * np.pi * 100 * t) + np.random.normal(0, 0.5, fs)
            for _ in range(20)
    ]
    ...
    for i in range(spp.shape[0]):
        ax.semilogx(f, 10*np.log10(spp[i]/2e-12), linewidth=0.5,
                 color="black", alpha=0.5)
    ax.semilogx(f, 10*np.log10(np.mean(spp, axis=0)/2e-12), linewidth=2,
             color="black")
    ax.semilogx(f, 10*np.log10(spp_avg/2e-12), linewidth=2,
             color="red")
    ax.set_title('Power Spectral Density')
    ax.set_xlabel('Frequency (Hz)')
    ax.set_ylabel(r'PSD (dB/Hz)')
    ax.grid()
    plt.savefig("wps_test_plot.png")
    plt.close()
    ...
    assert np.allclose(np.mean(spp, axis=0), spp_avg), "Mean of spp should equal spp_avg"
...
def test_coherence_length():
    ...
    # Compute the power spectral density using the spectrum function
    f, ly = preproc.coherence_length(pressure, z, ref_index=z.shape[0]//2,
                                      fs=fs, filter=False)
    ...
    ax.grid()
    plt.savefig("coherence_legth_test_plot.png")
    plt.close()
